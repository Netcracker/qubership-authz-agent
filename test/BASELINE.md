...
| Chart render | `helm template helm-templates/authz-agent` and `helm-templates/authz-policy-admin` | 0 | one Deployment with one container each | `test/scripts/test-chart-render.sh` asserts what both renders have to carry |
