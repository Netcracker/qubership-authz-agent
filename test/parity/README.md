...
| `TestRound15EqualityPairCases`, `…RelationalPairCases`, `…MembershipPairCases`, `…ContainsPairCases`, `…ContainsAnyPairCases`, `…SubsetPairCases`, `…MatchPairCases`, `…UnaryPairCases`, `…QuestionCases`, `…EntitlementTargetCases` | Every pair of operand kinds an operator may take that no earlier golden evaluated, one probe each, and the forms round 14 left open. Data, see [Cases kept as data](#cases-kept-as-data) | `load-simplified-policies-v1/isolated/`, `check-resource-v1-outcome/isolated/`, `load-policy-sets-v1/regular/`, `check-resource-v1-outcome/regular/`, `check-filter-v1-outcome/regular/` |
| `TestRound16HypothesesCases`, `…SyntaxCases`, `…SetsCases`, `…OrderCases`, `…SecondCarrierCases`, `…ResidualCases`, `…ValuesCases`, `…PipOperandsCases` | The value types each operator meets, a PIP holding the value an attribute would, cells of the operator-by-state table that one earlier golden fixes, how a string reads as a number, which characters `MATCH` matches, JSON Path through a list, condition syntax, and trees of sets no golden combined the same way. `…OrderCases` files each answer under its order class. Data, see [Cases kept as data](#cases-kept-as-data) | `load-simplified-policies-v1/isolated/`, `check-resource-v1-outcome/isolated/`, `load-policy-sets-v1/regular/`, `check-resource-v1-outcome/regular/`, `check-filter-v1-outcome/regular/` |
...
| `TestRound20QuestionsCases`, `…ReaskRound15GroupsCases`, `…ContextCases`, `…ContextOneGrantCases`, `…ContextTwoGrantsCases`, `…ContextWithoutMappingCases` | The operand contexts of round 19's context files, asked again with no empty rule condition; the two round 15 filter cases whose 400 was the stand state, in a file that uploads no PIPs; `subject allowed` and `subject denied` over a decision that fails, that the set decides by default, or that no set applies to; empty and blank rule, policy, and set targets and a blank rule condition; and a repeated predicate in two policies under distinct rule keys. Data, see [Cases kept as data](#cases-kept-as-data) | `check-resource-v1-outcome/regular/`, `check-resource-v1-outcome/isolated/`, `check-filter-v1-outcome/regular/`, `check-filter-v1-outcome/isolated/`, `load-policy-sets-v1/regular/`, `load-simplified-policies-v1/isolated/` |
...
| `TestRound25InactiveCases` | Where a set's `INACTIVE` status counts, for a check and a filter: a nested `INACTIVE` set that allows, that brings a predicate, and that denies beside a policy that allows or alone under `PERMIT_UNLESS_DENY`; an `INACTIVE` set two levels down and in the middle of a chain; a top-level `INACTIVE` set around an `ACTIVE` nested set. Data, see [Cases kept as data](#cases-kept-as-data) | `check-resource-v1-outcome/regular/`, `check-filter-v1-outcome/regular/`, `load-policy-sets-v1/regular/` |
| `TestRound26RightErrorFirstCases`, `…GeneralScalarCases`, `…ResidualCases`, `…HypothesesCases` | The comparisons no golden reached after round 25: a GENERAL PIP that fails on the right beside `null`, a GENERAL text body, a HEADER sent empty, an empty list or an empty selection on the left, for the operator and left side pairs not yet recorded; a number or boolean GENERAL body, mostly on the right of each operator; objects beside strings, a number beside a nested list, a HEADER list beside a list; and a nested list under `CONTAINS ANY` and `NOT CONTAINS ANY`. Each comparison is asked in an ALLOW rule and, where it may end the rule or fail the decision, in a DENY rule. Data, see [Cases kept as data](#cases-kept-as-data) | `check-resource-v1-outcome/regular/`, `check-resource-v1-outcome/isolated/`, `load-policy-sets-v1/regular/`, `load-simplified-policies-v1/isolated/` |
| `TestRound27ListsCases`, `…ResidualCases` | How a list inside a list compares: `[["v, w"]]` against `[["v", "w"]]`, which differ element by element and read the same as text, and two strings in another order and in the same order; and the four comparisons no golden reached after round 26: a selection holding `null` under `CONTAINS ANY`, bare `null` beside an index past the end under `>=` and `NOT CONTAINS ANY`, and an object beside a GENERAL PIP that answers `404` under `NOT CONTAINS ANY`. Each in an ALLOW rule and in a DENY rule. Data, see [Cases kept as data](#cases-kept-as-data) | `check-resource-v1-outcome/regular/`, `load-policy-sets-v1/regular/` |
| `TestRound28SubjectResourceTypeCases`, `…SubjectUrlCases`, `…SubjectCaseCases`, `…SubjectRealmCases` | How a value that holds characters the filter dialects or a URL reserve reaches a filter predicate and a GENERAL PIP's url: `${resourceType}` in the predicates of all four dialects, for resource types that hold such characters; a GENERAL url that ends with `${resource.u}`, with a pip-mock route for each plausible reading of the url; `subject.name` and `subject.type` compared with their text in another case; and `${subject.id}` and `${subject.name}` for realm users whose id holds such characters; their usernames are plain, because Keycloak refuses those characters in a username. `…SubjectRealmCases` needs the users of `suite/testdata/realm/round28-subject-users.json` in the parity realm and fails on the first token without them. The file is the realm's `users` array: wrap it as `{"users": …}` and add it with the realm's partial import, which keeps the given ids. Data, see [Cases kept as data](#cases-kept-as-data) | `check-filter-v1-outcome/regular/`, `check-resource-v1-outcome/regular/`, `check-resource-v1-outcome/isolated/`, `load-policy-sets-v1/regular/`, `load-simplified-policies-v1/isolated/` |
| `TestRound28PipHeaderCases`, `…PipTokenCases`, `…PipMappingCases` | What a PIP declaration may name, which the earlier cases never varied: a HEADER PIP over a header named in another case, with spaces around it, or empty, or over one the client sends itself such as `Authorization` or `Host`, and whether an empty header takes `defaultValue`; a TOKEN PIP with an empty claim; and a MAPPING key over every kind of source, including a GENERAL PIP that answers a list, a scalar, `null`, `[]`, `404`, or `500`, another MAPPING PIP, and `subject.permissions` itself, read in a simplified policy, in a rule condition and a rule target, in a DENY rule, under `iterate`, and as a filter placeholder. A header whose value no case chooses is only asked `IS NOT NULL`. Data, see [Cases kept as data](#cases-kept-as-data) | `check-filter-v1-outcome/regular/`, `check-resource-v1-outcome/regular/`, `check-resource-v1-outcome/isolated/`, `load-policy-sets-v1/regular/`, `load-simplified-policies-v1/isolated/` |
| `TestRound28RsqlNegationCases`, `…RsqlSubstitutionCases` | How `check/filter` writes the negation of a DENY predicate under `DENY_OVERRIDES` and `PERMIT_UNLESS_DENY`: an RSQL comparison operator of each family, `;` and `,` inside one predicate, groups, and an `=in=` list that a GENERAL PIP leaves empty; and how substituted values that hold `,`, `;`, quotes, parentheses, `==`, or a backslash are written, from GENERAL, HEADER, TOKEN, MAPPING, `subject.roles`, and `resourceType`. Each file opens and closes with the control `blocked==1`. Data, see [Cases kept as data](#cases-kept-as-data) | `check-filter-v1-outcome/regular/`, `load-policy-sets-v1/regular/` |
| `TestRound28ReachValuesCases`, `…ReachSetsCases`, `…ReachScopeNoPipCases`, `…ReachScopeTextCases` | Inputs no golden reached: a rule target or condition sent as JSON `null`, a number beyond the range of a double, jsonPath functions over a selection, an index into an object, filters over a scalar or a missing key; `check/filter` over a simplified policy of another operation, over a target that fails, and over an `iterate` whose `foreach` is not the permission scope; and an iterating set on a stand with no permission scope PIP, or with a scope service that answers text that is not JSON. `…ReachScopeNoPipCases` declares only a HEADER PIP in its domain and pins the scope route with a grant, recording its calls, so a scope PIP left on the stand by an earlier run shows in the goldens. Each file opens and closes with a control. Data, see [Cases kept as data](#cases-kept-as-data) | `check-filter-v1-outcome/regular/`, `check-filter-v1-outcome/isolated/`, `check-resource-v1-outcome/regular/`, `check-resource-v1-outcome/isolated/`, `load-policy-sets-v1/regular/`, `load-simplified-policies-v1/isolated/` |
...
`TestInterpreterBarePathCases`, `TestInterpreterPIPDeclarationCases` and the `TestRound9*`, `TestRound10*`, `TestRound11*`, `TestRound13*`, `TestRound14*`, `TestRound15*`, `TestRound16*`, `TestRound17*` and `TestRound18*` functions
...
differs between stands, and an answer that depends on whether a failing PIP was reached is recorded with the order the
