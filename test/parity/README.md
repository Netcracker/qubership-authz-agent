...
| `TestRound11ScopeOutsideIterateCases` | What `subject.permissionScope.region` resolves to in a set that does not iterate, under `IS EMPTY` alone and on the left of an `OR`, with `IS NULL` as the control. Legacy profile only | `load-policy-sets-v1/regular/`, `check-resource-v1-outcome/regular/` |
...
`TestInterpreterBarePathCases`, `TestInterpreterPIPDeclarationCases` and the `TestRound9*`, `TestRound10*` and `TestRound11*` functions
