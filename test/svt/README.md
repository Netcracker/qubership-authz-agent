...
> **One container under test.** The stack used to run Envoy, OPA, pap-client, and a decision-log
> collector as separate services, and the reports carried one column group per container. Since
> authz-agent-ADR-0080 the agent is one process, so the stand boots `authz-agent` beside Keycloak
> and the policy stub, and the reports carry one group. The three modes are unchanged in meaning:
> `canonical` and `legacy` reach the public surface on `8080`, `opa-direct` reaches the data API on
> `8181`, and authz-agent-ADR-0062 pins the two transports as byte-identical.
...
│   ├── compose/
│   │   ├── docker-compose.yml         # SVT stack (authz-agent, authz-policy-admin, keycloak, cadvisor, prometheus, grafana, jmeter)
│   │   ├── keycloak/
│   │   │   ├── svt-realm.json
...
│   ├── full/                          # public-surface tests (canonical + legacy pairs)
│   │   ├── mixed/
│   │   │   ├── 100rps/
│   │   │   ├── 200rps/
│   │   │   ├── 300rps/
...
│   ├── up
│   ├── down
│   ├── run                            # legacy public-surface runner
...
tests/svt/scripts/run-individual-svt-matrix --transport-modes public-canonical,public-legacy,opa-direct --scenarios ols-single,rls-filter --target-rps 500,1000 --output-dir tests/svt/load-tests/individual/artifacts/example
tests/svt/scripts/run-individual-svt-matrix --transport-modes public-legacy --scenarios rls-filter --target-rps 25 --threads 1 --ramp-seconds 1 --duration-seconds 6 --output-dir tests/svt/load-tests/individual/artifacts/smoke
...
### Full (public surface) — canonical + legacy pairs
...
- Transport modes: `public-canonical`, `public-legacy`, `opa-direct`
...
- queries Prometheus/cAdvisor for the agent's CPU/memory samples over the exact measured JTL
  time window;
...
Bootstrap (`tests/svt/scripts/up`) and `svt_restart_agent` merge base seed +
...
Bootstrap (`tests/svt/scripts/up`) and `svt_restart_agent` merge the
...
| ------------------------ | ------------- | ----------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| `authz-agent`            | 8080 / 8181   | 28080 / —         | The service under test: public surface on `8080`, data API on `8181`, which the `opa-direct` profile targets from Compose only |
| `authz-policy-admin`     | 18090         | 28093             | Policy source the agent pulls from, and what the seeds are uploaded to                                                         |
