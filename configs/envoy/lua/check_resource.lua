...
local skip_headers = {
  ...
  -- ADR-0049: incoming-token must be stripped from HEADER PIP resolution
  -- even when the Lua filter reads it as the legacy subject source,
  -- so a misconfigured HEADER PIP cannot leak the raw bearer string.
  ["incoming-token"] = true,
}
...
-- ADR-0049: on legacy ingress, the admission token comes from Authorization
-- (M2M), and the policy-evaluation subject token comes from Incoming-Token
-- (end-user) when present, else falls back to Authorization, else empty.
-- When Authorization-Type: anonymous is set, the subject is the unauthenticated
-- marker (empty string) regardless of which headers the client sent.
...
-- Extract the raw JSON value of a field that is a quoted string, returning the unquoted content.
...
function envoy_on_request(request_handle)
  ...
  -- (separate from the stripped subject header blob, so HEADER-PIP stripping in
  -- skip_headers is preserved). On the Envoy path Envoy always supplies one
  -- (generated if the client omitted it); empty here => OPA generates its own.
  ...
  -- Row-11 (Validation) parity: the legacy CheckRequestValidator
  -- (CheckRequestValidator.java:31-44) rejects missing required parameters
  -- with HTTP 400 and a message that names the missing field. We emit a
  -- body whose substring matches the parity test's tolerance set
  -- ("operation"/"parameter"/"invalid") so the case asserts correctly
  -- without taking an unrelated short-circuit through the canonical
  -- bad-request body.
  ...
  -- Extract legacy fields and build canonical OPA input.
  ...
  request_handle:headers():replace("x-authz-original-path", "/access/v1/check/resource")
end
...
function envoy_on_response(response_handle)
  ...
  -- Derive boolean from canonical AuthorizeResponse: results[0].isAllowed == true
  local results_json = extract_field_json(unwrapped, "results")
  local allowed = false
  if results_json ~= nil then
    -- Extract first result object from the results array.
  ...
  if allowed then
    write_json(response_handle, "true")
  else
    write_json(response_handle, "false")
  end
end
